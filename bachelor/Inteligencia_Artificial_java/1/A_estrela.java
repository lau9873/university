...
public class A_estrela {
    ...
    public static void Start(int[][] configInicial, int[][] configFinal){
        ...
        InfoNode<int[][]> node = new InfoNode<int[][]>(configInicial,null,0);
        heuristicsA(node);
    ...
    public static void Search(InfoNode<int[][]> node, LinkedListStack<InfoNode<int[][]>> pilha, MinHeap<InfoNode<int[][]>> sonAux){
        ...
        if (Arrays.deepEquals(node.getValue(),ConfigFinal)){ solution = Pathtosolution(pilha);}
        ...
        //order(sonAux1);
        while (!sonAux1.isEmpty()){
            InfoNode<int[][]> test = sonAux1.removeMin();
            if (node.getDepth() == 0) Search(test,pilha,sonAux1);
            if (cost != 0 && test.getH() < cost) Search(test,pilha,sonAux1);
            //if (sonAux.size() != 0) System.out.println(sonAux.min().getH());
            if (cost == 0 && sonAux.size() != 0 && test.getH() <= sonAux.min().getH()){
                Search(test,pilha,sonAux1);
                pilha.pop();
            }
