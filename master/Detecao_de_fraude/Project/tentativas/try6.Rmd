...
```{r}
dados_featured<-reducao_pca_p
percentagem_treino <- 0.7
set.seed(1234)
split_dados <- dados_featured %>% initial_split(prop=percentagem_treino,strata=is_fraud)
...
percentagem_dados <- c(0.01, 0.05, 0.1, 0.2, 0.3)
...
for (num_componentes in num_componentes_lista){
  ...
  for (f in 1:6){
    ...
    for (p in 1:5){
      percentagem_total <- percentagem_dados[p]
      
      # Divisão em conjunto de treino e teste
      percentagem_treino <- percentagem_total
      set.seed(1234)
      split_dados <- dados_featured %>% initial_split(prop=percentagem_treino,is_fraud)
      dados_treino <- training(split_dados)
      dados_teste <- testing(split_dados)
...
for (i in 1:nrow(matriz)) {
  dados <- t(matriz[i, , ])
  matplot(percentagem_dados*100, dados, type = "l", col = cores, xlab = "Percentagem dos dados para treino",
...
for (i in 1:nrow(matriz)) {
  dados <- t(matriz[i, , ])
  matplot(percentagem_dados[-6]*100, dados[,-6], type = "l", col = cores, xlab = "Percentagem dos dados para treino",
          ylab = "Percentagem de erros", main = paste("Feature", dados_featured_nome_lista[i]))
