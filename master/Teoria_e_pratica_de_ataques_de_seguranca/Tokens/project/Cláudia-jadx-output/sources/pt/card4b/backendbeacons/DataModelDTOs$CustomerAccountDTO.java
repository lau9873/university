...
public class DataModelDTOs$CustomerAccountDTO {
    ...
    public DataModelDTOs$CustomerAccountDTO(DataModels.UserAccountInfo userAccountInfo) {
        this.ID = String.valueOf(userAccountInfo.getId());
        ...
        if (userAccountInfo.getProfileInfo() != null) {
            ...
            for (DataModels.UserProfileInfo userProfileInfo : userAccountInfo.getProfileInfo()) {
                if (userProfileInfo.getID() != null) {
                    DataModels.CustomerProfile customerProfile = l.k().i().get(userProfileInfo.getProfileId());
    ...
    public DataModels.UserAccountInfo toAccountInfo() {
        ...
        if (getBillingInformationDetails() == null || getBillingInformationDetails().getID() == null) {
            ...
            String id = getBillingInformationDetails().getID();
            String name = getBillingInformationDetails().getName();
            String address = getBillingInformationDetails().getAddress();
            String city = getBillingInformationDetails().getCity();
            String postalCode = getBillingInformationDetails().getPostalCode();
            String fiscalNumber = getBillingInformationDetails().getFiscalNumber();
            DataModels.Country country = (getBillingInformationDetails().getCountryDetails() == null || getBillingInformationDetails().getCountryDetails().getId() == null) ? null : new DataModels.Country(getBillingInformationDetails().getCountryDetails().getId(), getBillingInformationDetails().getCountryDetails().getCode(), getBillingInformationDetails().getCountryDetails().getName());
            str = getBillingInformationDetails().NotifyEmail;
            userBillingInfo = new DataModels.UserBillingInfo(id, name, address, city, postalCode, fiscalNumber, country, str);
        }
        if (getCustomerProfilesDetails() == null || getCustomerProfilesDetails().size() <= 0) {
            ...
            for (DataModelDTOs$CustomerProfileDetailDTO dataModelDTOs$CustomerProfileDetailDTO : getCustomerProfilesDetails()) {
                if (dataModelDTOs$CustomerProfileDetailDTO.getID() != null) {
                    DataModels.CustomerProfile customerProfile = l.k().i().get(dataModelDTOs$CustomerProfileDetailDTO.getProfileId());
